...
run_program(){
   ...
   [ ! -z ${RUNONLY} ] &&  ./${RUNCMD}.jar run || (./${RUNCMD}.jar start;/usr/local/nginx/sbin/nginx -g "daemon off;")
}
...
_main() {
   ...
   if [ $# -gt 2 ];then
      loginfo_error "Too many arguments; maximum of 2 parameters"
   fi
   ...
   getEnv "$@"
   if [ a"$2" == a"only" ];then
      loginfo_note "Only Start APP: $1"
   else
      loginfo_note "Start program $1 and Nginx"
   fi
   run_program "$@"
}
